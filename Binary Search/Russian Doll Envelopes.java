```
/*
Description
You have a number of envelopes with widths and heights given as a pair of integers (w, h). 
One envelope can fit into another if and only if both the width and height of one envelope is greater than 
the width and height of the other envelope.

What is the maximum number of envelopes can you Russian doll? (put one inside other)

Example:
Given envelopes = [[5,4],[6,4],[6,7],[2,3]], the maximum number of envelopes you can Russian doll is 3 
([2,3] => [5,4] => [6,7]).

Thoughts
Solution 1: O(n * log (n)) time and O(n) space
1. Sort the envelopes: ascend on width and descend on height to break tie. O(n * log (n)) time and O(1) space
2. Find the longest increasing subsequence based on height. O(n * log (n)) time and O(n) space


*/

// Solution 1

```
